...
get_ipython().system('gdown 1gew1zSfSZKiUKGPGc3bpGXbO03HvE9-_')


# In[ ]:


get_ipython().system('unzip  SignatureObjectDetection')
...
img_dir="/kaggle/working/TrainImages"
label_dir="/kaggle/working/TrainGroundTruth"
...
img_dir="/kaggle/working/TrainImages"
label_dir="/kaggle/working/TrainGroundTruth"
images = [image for image in sorted(os.listdir(img_dir))
                        if image[-4:]=='.tif']
annots=[]
for image in images:
    annot = image[:-4] + '.txt'
...
img_dir2="/kaggle/working/TestImages"


# In[138]:


images2 = [image for image in sorted(os.listdir(img_dir2))
                        if image[-4:]=='.tif']
annots2=[]
for image in images2:
    annot = image[:-4] + '.txt'
    annots2.append(annot)
...
images2 = pd.Series(images2, name='images')
annots2 = pd.Series(annots2, name='annots')
df2 = pd.concat([images2, annots2], axis=1)
...
class ImagesDataset(torch.utils.data.Dataset):
    def __init__(self, df=df, files_dir1="/kaggle/working/TrainGroundTruth",files_dir2="/kaggle/working/TrainImages", S=7, B=2, C=1, transform=None):
        self.annotations = df
        self.files_dir1 = files_dir1
        self.files_dir2 = files_dir2
    ...
    def __getitem__(self, index):
        ...
        img_path = os.path.join(self.files_dir2, self.annotations.iloc[index, 0])
        image = Image.open(img_path)
        image = image.convert("RGB")
        img_width, img_height = image.size
        label_path = os.path.join(self.files_dir1, self.annotations.iloc[index, 1])
        boxes = []
        ...
        with open(label_path) as f:
            ...
            for x in label.replace(",", " ").split()
                ]
                
                
        boxes.append([0,((xmax + xmin) / 2) / img_width,((ymax + ymin) / 2) / img_height , (xmax - xmin) / img_width, (ymax - ymin) / img_height ])
...
def show_objects(img, detections):
    ...
    img = np.array(img)
    plt.figure()
    fig, ax = plt.subplots(1, figsize=(448,448))
    ax.imshow(img)
...
label_path = os.path.join("/kaggle/working/TrainGroundTruth",df.iloc[7, 1])
...
d1,d2,d3,d4=z[0][3],z[0][4],z[0][5],z[0][6]
